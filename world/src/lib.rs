...
impl World {
    ...
    pub fn new() -> Self {
        ...
        let wall = wall_aligned_with_grid(tile_grid.columns(), tile_grid.rows(), cells_per_tile);
...
pub fn apply(world: &mut World, command: Command, out_events: &mut Vec<Event>) {
    match command {
        Command::ConfigureTileGrid {
            ...
            world.wall = wall_aligned_with_grid(columns, rows, normalized_cells);
...
pub mod query {
    ...
    pub fn occupancy_view(world: &World) -> OccupancyView<'_> {
        ...
        OccupancyView::new(columns, rows, world.occupancy.cells())
    }
...
fn wall_aligned_with_grid(columns: TileCoord, rows: TileCoord, cells_per_tile: u32) -> Wall {
    let cells = target_cells(columns, rows, cells_per_tile);
    Wall::new(Target::new(cells))
}
...
mod tests {
    use super::*;
    use maze_defence_core::{BugColor, Goal, PlayMode, TowerSnapshot};
    use std::{
    ...
    impl From<TowerSnapshot> for TowerRecord {
        fn from(snapshot: TowerSnapshot) -> Self {
            Self {
                id: snapshot.id,
                kind: snapshot.kind,
